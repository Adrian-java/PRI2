...
public class VisitRestController {
    ...
    /**
     * Service injected by Spring that provides CRUD operations for Visit
     * entities
     * 
     */
    @Autowired
	...
	binder.registerCustomEditor(boolean.class,
		new org.skyway.spring.util.databinding.EnhancedBooleanEditor(false));
	binder.registerCustomEditor(Boolean.class,
		new org.skyway.spring.util.databinding.EnhancedBooleanEditor(true));
	...
	return Response.ok(visitService.deleteVisitTypeOfVisit(visit_id, related_typeofvisit_id))
		.build();
    }
	...
	return Response.ok(visitService.deleteVisitSickLeaves(visit_id, related_sickleaves_id))
		.build();
    }
    ...
    public Response loadVisitTypeOfVisit(@PathParam("visit_id") Integer visit_id,
	    @PathParam("related_typeofvisit_id") Integer related_typeofvisit_id) {
	TypeOfVisit typeofvisit = typeOfVisitDAO.findTypeOfVisitByPrimaryKey(
		related_typeofvisit_id, -1, -1);
	...
	try {
	    Doctor d = systemUserDao.findSystemUserByPesel(visitmodel.getDoctorLogin()).getWorker()
		    .getDoctor();
	    Patient p = systemUserDao.findSystemUserByPesel(visitmodel.getPatientPesel())
		...
		pc.setDoctor(d);
		pc.setPatient(p);
		pc.setRegisterDate(systemUserDao.findSystemUserByPesel(visitmodel.getDoctorLogin())
			.getRegisterDate());
		Integer id = patientCardService.savePatientCard(pc);
	    }
	    Receptionist r = systemUserDao.findSystemUserByPesel(visitmodel.getRecepcionistLogin())
		    .getWorker().getReceptionist();
	    StatusOfVisit sov = statusOfVisitDAO
		    .findStatusOfVisitByType(visitmodel.getStatusOfVisit()).iterator().next();
	    TypeOfVisit tof = typeOfVisitDAO.findTypeOfVisitByName(visitmodel.getTypeOfVisit())
		    .iterator().next();
	...
	return Response.ok(visitService.deleteVisitReceptionist(visit_id, related_receptionist_id))
		.build();
    }
	...
	return Response.ok(statusOfVisitDAO.findStatusOfVisitByPrimaryKey(statusofvisit.getId()))
		.build();
    }
	...
	return Response.ok(visitService.deleteVisitPatientCard(visit_id, related_patientcard_id))
		.build();
    }
	...
	return Response.ok(receptionistDAO.findReceptionistByPrimaryKey(receptionist.getId()))
		.build();
    }
	...
	return Response.ok(
		visitService.deleteVisitStatusOfVisit(visit_id, related_statusofvisit_id)).build();
    }
    ...
    @PUT
    public Response saveVisitDoctor(@PathParam("visit_id") Integer visit_id, Doctor doctor) {
    ...
    @Path("/get/{pesel}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getVisitByPesel(@PathParam("pesel") String pesel)
	    throws JsonGenerationException, JsonMappingException, DataAccessException, IOException {
	return Response.ok(
		new ObjectMapper().configure(Feature.FAIL_ON_EMPTY_BEANS, false)
			.writeValueAsString(visitDAO.findVisitByPesel(pesel))).build();
    }
	...
	for (Visit v : visit) {
		...
		if (su.getWorker().getPatient() != null
			&& v.getPatientCard().getPatient().getId()
				.equals(su.getWorker().getPatient().getId())) {
    ...
    public Response loadVisitPatientCard(@PathParam("visit_id") Integer visit_id,
	    @PathParam("related_patientcard_id") Integer related_patientcard_id) {
	PatientCard patientcard = patientCardDAO.findPatientCardByPrimaryKey(
		related_patientcard_id, -1, -1);
    ...
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public Response newVisitSickLeaves(@PathParam("visit_id") Integer visit_id, SickLeave sickleave) {
    ...
    public Response loadVisitStatusOfVisit(@PathParam("visit_id") Integer visit_id,
	    @PathParam("related_statusofvisit_id") Integer related_statusofvisit_id) {
	StatusOfVisit statusofvisit = statusOfVisitDAO.findStatusOfVisitByPrimaryKey(
		related_statusofvisit_id, -1, -1);
    ...
    public Response loadVisitReceptionist(@PathParam("visit_id") Integer visit_id,
	    @PathParam("related_receptionist_id") Integer related_receptionist_id) {
	Receptionist receptionist = receptionistDAO.findReceptionistByPrimaryKey(
		related_receptionist_id, -1, -1);
    ...
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public Response newVisitDoctor(@PathParam("visit_id") Integer visit_id, Doctor doctor) {
	visitService.saveVisitDoctor(visit_id, doctor);
	return Response.ok(doctorDAO.findDoctorByPrimaryKey(doctor.getId())).build();
    }
