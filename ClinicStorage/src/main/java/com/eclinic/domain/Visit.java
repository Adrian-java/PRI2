...
@NamedQueries({
		...
		@NamedQuery(name = "findVisitByPesel", query = "select v from Visit v where v.patient in (select p from Patient p where  p in (select w.patient from Worker w where w in (select su.worker from SystemUser su where su.pesel =?1 ))))"),
