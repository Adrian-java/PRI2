...
public class VisitCrudDb implements VisitCrud {
	...
	Set<VisitProposal> findFreeTermsByDoctor = findFreeTermsByDoctor(id, null);
	return (findFreeTermsByDoctor.isEmpty() ? null : findFreeTermsByDoctor.iterator().next());
    }
	...
	Set<VisitProposal> findFreeTermsBySpecialization = findFreeTermsBySpecialization(
		specialization, null);
	return (findFreeTermsBySpecialization.isEmpty() ? null : findFreeTermsBySpecialization
		.iterator().next());
    }
	...
	Set<Visit> findVisitByDateOfVisitAfter = visitDao.findVisitByDateOfVisitAfter(calendar);
	...
	Set<VisitProposal> visitBySpecialization = visitHelper.getFreeVisitBySpecialization(
		findVisitSchedulerBySpecialization, findVisitByDateOfVisitAfter);
	...
	v.setReceptionist(receptionistDao.findReceptionistById(newVisit.getIdReceptionist()));
	...
	v.setStatusOfVisit((findStatusOfVisitByType == null ? null : findStatusOfVisitByType
		.iterator().next()));
	Set<TypeOfVisit> findTypeOfVisitByName = typeOfVisitDao.findTypeOfVisitByName(newVisit
		.getTypeOfVisit());
	v.setTypeOfVisit((findTypeOfVisitByName == null ? null : findTypeOfVisitByName.iterator()
		.next()));
